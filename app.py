import os, re, io, zipfile, uuid, traceback
...
<h1>&#128663; Ped&aacute;gio App</h1><p class="sub">Comprovantes PDF &rarr; planilha no padr&atilde;o do gabarito</p>
<div class="card">
<label>&#128193; Planilha Modelo (.xlsx)</label><input type="file" id="modelo" accept=".xlsx">
<label>&#128196; Comprovantes PDF (v&aacute;rios)</label><input type="file" id="pdfs" multiple>
<div class="stats">
...
async function proc(){
  ...
  var btn=document.getElementById('btn');btn.disabled=true;btn.textContent='Processando...';
  document.getElementById('log').textContent='Enviando...';
  var fd=new FormData();fd.append('modelo',mo);for(var i=0;i<pf.length;i++)fd.append('pdfs',pf[i]);
  try{
    var r=await fetch('/api/processar',{method:'POST',body:fd});var j=await r.json();
